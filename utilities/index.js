...
Util.buildInventoryDetails = async function(data){
  ...
  if(data.length > 0){
    details = '<div class="row">'
      ...
      details +=  '<h3 id="des"><span>Description: </span>'+ data[0].inv_description + '</h3>' 
      details += '<h3 id="price">'
      details += '<span>Price: $' 
      + new Intl.NumberFormat('en-US').format(data[0].inv_price) + '</span>'
      details += '</h3>'
      details += '<h3><span>Make: </span>'
       + data[0].inv_make +'</h3>' 
       details += '<h3><span>Model: </span>'
      + data[0].inv_model + '</h3>' 
      details += '<h3><span>Color: </span>'
      + data[0].inv_color + '</h3>' 
      details += '<h3><span>Year: </span>'
      + data[0].inv_year + '</h3>'
      details += '<h3><span>Milage: </span>'
      + new Intl.NumberFormat('en-US').format(data[0].inv_miles) + '</h3>'          
    details += '</section>' 
    details += '</div>'          
